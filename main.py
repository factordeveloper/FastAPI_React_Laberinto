...
class Laberinto:
    ...
    def _resolver_recursivo(self, x: int, y: int, visitados: Set[Tuple[int, int]], camino_actual: List[Tuple[int, int]]):
        ...
        if (x, y) != self.inicio and (x, y) != self.fin:
            self.laberinto[x][y] = '@'  # Marca la posición actual de la tortuga
        self._imprimir_laberinto()
        if (x, y) != self.inicio and (x, y) != self.fin:
            self.laberinto[x][y] = 'o'  # Deja la marca del camino
        ...
        camino_actual.pop()
        if (x, y) != self.inicio and (x, y) != self.fin:
            self.laberinto[x][y] = valor_original  # Restaurar el valor original
            self._imprimir_laberinto()
    ...
    def _imprimir_solucion(self, camino: List[Tuple[int, int]]):
        # Crear una copia del laberinto original
